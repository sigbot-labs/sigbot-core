...
use crate::client::cex::ISigbotCEXExchangeClient;
...
impl ISigbotExchangeClient for SigbotBinanceClient {
    ...
    async fn init(&self) {
        ...
        if self.config.use_websocket {
            debug!("Initializing USDS API WS client with config={}", self.config);
            ...
            debug!("Initializing USDS Market WS client with config={}", self.config);
            match DerivativesTradingUsdsFuturesWsApi::from_config(
                ConfigurationWebsocketApi::builder()
                    .api_key(self.config.api_key.clone())
                    .api_secret(self.config.api_secret.clone())
            ...
            debug!("Initializing USDS Market WS streams with config={}", self.config);
            ...
            *self.ws_market_stream.lock().await = Some(connection);
            ...
            debug!("Initializing USDS API Rest client with {}", self.config);
            ...
            *self.rest_api_client.lock().await = Some(rest_api);
            ...
            debug!("Initializing USDS Market API Rest client with {}", self.config);
...
#[async_trait]
impl ISigbotOrderBookExchangeClient for SigbotBinanceClient {
...
#[async_trait]
impl ISigbotCEXExchangeClient for SigbotBinanceClient {}
