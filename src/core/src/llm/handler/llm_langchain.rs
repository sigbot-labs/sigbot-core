...
impl LangchainOperation {
    pub async fn new() -> Arc<Self> {
        ...
        if let Some(api_key) = &config::get_config().llm.embedding.api_key {
        ...
        if let Some(org_id) = &config::get_config().llm.embedding.org_id {
            embedding_openai_config = embedding_openai_config.with_org_id(org_id);
        }
        if let Some(project_id) = &config::get_config().llm.embedding.project_id {
        ...
        let call_opts = CallOptions::new()
            .with_max_tokens(config::get_config().llm.generate.max_tokens)
            .with_temperature(config::get_config().llm.generate.temperature)
            .with_candidate_count(config::get_config().llm.generate.candidate_count)
            // TODO: whether the support configuration of this items?
            .with_functions(Vec::new())
            .with_stop_words(Vec::new())
            .with_top_k(config::get_config().llm.generate.top_k)
            .with_top_p(config::get_config().llm.generate.top_p)
            // .with_seed(0)
...
impl ILLMOperation for LangchainOperation {
    ...
    async fn generate(&self, prompt: String) -> Result<String, anyhow::Error> {
        ...
        //     tool_calls: None,
        // }];
        // let embedding_result = ChatCompletion::builder(&config::get_config().sigbot.llm.embedding.model, messages.clone())
...
Helpful Answer:
        ...
        let opts = VecStoreOptions::new()
            .with_name_space("sigbot") // TODO: namespace
