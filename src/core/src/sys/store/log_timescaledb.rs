...
use crate::store::timescaledb::TimescaleDBRepository;
use crate::store::IAsyncRepository;
// Macros are exported at crate root with #[macro_export]
...
pub struct LogTimescaleDBRepository {
    inner: TimescaleDBRepository<LogInfo>,
}

impl LogTimescaleDBRepository {
    pub async fn new(config: &TimescaleDBAppDBProperties) -> Result<Self, Error> {
        Ok(LogTimescaleDBRepository {
            inner: TimescaleDBRepository::get_or_init(config).await?,
        })
    }
...
#[async_trait]
impl IAsyncRepository<LogInfo> for LogTimescaleDBRepository {
    async fn select(&self, log: LogInfo, page: PageRequest) -> Result<(PageResponse, Vec<LogInfo>), Error> {
        // Build WHERE clause dynamically
        let mut conditions = Vec::new();
        let mut param_index = 1;

        if log.service_name.is_some() {
            conditions.push(format!("service_name = ${}", param_index));
            param_index += 1;
        }
        if log.log_type.is_some() {
        ...
        let count_query = format!(
            "SELECT COUNT(1) as count FROM sys_log WHERE {} AND del_flag = 0",
            where_clause
        );
        let mut count_stmt = sqlx::query(&count_query);
        if let Some(ref service_name) = log.service_name {
            count_stmt = count_stmt.bind(service_name);
        }
        if let Some(ref log_type) = log.log_type {
            count_stmt = count_stmt.bind(log_type);
        }
        if let Some(ref level) = log.level {
            count_stmt = count_stmt.bind(level);
        }
        if let Some(ref content) = log.content {
            count_stmt = count_stmt.bind(format!("%{}%", content));
        }
        ...
        let data_query = format!(
            "SELECT * FROM sys_log WHERE {} AND del_flag = 0 ORDER BY created_at DESC LIMIT ${} OFFSET ${}",
            where_clause,
            param_index,
            param_index + 1
        );
        ...
        if let Some(ref service_name) = log.service_name {
            data_stmt = data_stmt.bind(service_name);
        }
        if let Some(ref log_type) = log.log_type {
            data_stmt = data_stmt.bind(log_type);
        }
        if let Some(ref level) = log.level {
            data_stmt = data_stmt.bind(level);
        }
        if let Some(ref content) = log.content {
            data_stmt = data_stmt.bind(format!("%{}%", content));
        }
    ...
    async fn select_by_id(&self, id: i64) -> Result<LogInfo, Error> {
        let log = sqlx::query_as::<_, LogInfo>("SELECT * FROM sys_log WHERE id = $1 AND del_flag = 0")
            .bind(id)
    ...
    async fn upsert(&self, mut log: LogInfo) -> Result<i64, Error> {
    ...
    async fn update(&self, _log: LogInfo) -> Result<i64, Error> {
        // Logs are append-only, updates are not supported
        Err(Error::msg("Log updates are not supported"))
    }

    async fn delete_all(&self) -> Result<u64, Error> {
        let result = sqlx::query("DELETE FROM sys_log").execute(self.get_pool()).await?;
        Ok(result.rows_affected())
    }

    async fn delete_by_id(&self, id: i64) -> Result<u64, Error> {
        let result = sqlx::query("UPDATE sys_log SET del_flag = 1 WHERE id = $1")
            .bind(id)
...
#[async_trait]
impl ILogRepository for LogTimescaleDBRepository {
    async fn append(&self, log: LogInfo) -> Result<i64, Error> {
        // It's specifically designed for append-only, so we don't support update.
        IAsyncRepository::<LogInfo>::upsert(self, log).await
    }
    ...
    async fn stats(
        ...
        if service_name.is_some() {
            conditions.push("service_name = $1".to_string());
        }
        if log_type.is_some() {
            conditions.push("log_type = $2".to_string());
        }
        if start_time.is_some() {
