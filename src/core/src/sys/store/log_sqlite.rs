...
#[async_trait]
impl IAsyncRepository<LogInfo> for LogSQLiteRepository {
...
#[async_trait]
impl ILogRepository for LogSQLiteRepository {
    ...
    async fn tail(
        ...
        if log_type.is_some() {
            conditions.push("log_type = ?".to_string());
        }
        if keyword.is_some() {
    ...
    async fn search(
        ...
        // Similar implementation to postgres but with SQLite syntax
        let mut conditions = Vec::new();
        conditions.push("del_flag = 0".to_string());
