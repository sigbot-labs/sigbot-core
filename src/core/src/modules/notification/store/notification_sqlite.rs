...
pub struct NotificationInfoSQLiteRepository {
    inner: SQLiteRepository<NotificationInfo>,
}

impl NotificationInfoSQLiteRepository {
    pub async fn new(config: &SqliteAppDBProperties) -> Result<Self, Error> {
        Ok(NotificationInfoSQLiteRepository {
...
#[async_trait]
impl IAsyncRepository<NotificationInfo> for NotificationInfoSQLiteRepository {
    async fn select(
        &self,
    ...
    async fn upsert(&self, mut notification: NotificationInfo) -> Result<i64, Error> {
        let upserted_id = dynamic_sqlite_upsert!(notification, "t_notification", self.inner.get_pool())?;
        info!("Inserted notification.id: {:?}", upserted_id);
        Ok(upserted_id)
    }

    async fn update(&self, mut notification: NotificationInfo) -> Result<i64, Error> {
        let updated_id = dynamic_sqlite_update!(notification, "t_notification", self.inner.get_pool())?;
