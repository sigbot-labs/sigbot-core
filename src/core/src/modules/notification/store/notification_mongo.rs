...
pub struct NotificationInfoMongoRepository {
    #[allow(unused)]
    inner: Arc<MongoRepository<NotificationInfo>>,
    collection: Collection<NotificationInfo>,
}

impl NotificationInfoMongoRepository {
    pub async fn new(config: &MongoAppDBProperties) -> Result<Self, Error> {
        let inner = Arc::new(MongoRepository::new(config).await?);
        let collection = inner.get_database().collection("t_notification");
        Ok(NotificationInfoMongoRepository { inner, collection })
    }
}
...
impl IAsyncRepository<NotificationInfo> for NotificationInfoMongoRepository {
    ...
    async fn select_by_id(&self, id: i64) -> Result<NotificationInfo, Error> {
        ...
        let notification = self
            ...
            .find_one(filter)
            .await?
            .ok_or_else(|| Error::msg("NotificationInfo not found"))?;
        Ok(notification)
    }
    ...
    async fn update(&self, mut notification: NotificationInfo) -> Result<i64, Error> {
        dynamic_mongo_update!(notification, self.collection)
    }
