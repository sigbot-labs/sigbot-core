...
pub struct BacktestCaseInfoPostgresRepository {
    inner: PostgresRepository<BacktestCaseInfo>,
}

impl BacktestCaseInfoPostgresRepository {
    pub async fn new(config: &PostgresAppDBProperties) -> Result<Self, Error> {
        Ok(BacktestCaseInfoPostgresRepository {
...
#[async_trait]
impl IAsyncRepository<BacktestCaseInfo> for BacktestCaseInfoPostgresRepository {
    async fn select(
        &self,
    ...
    async fn upsert(&self, mut backtest_case: BacktestCaseInfo) -> Result<i64, Error> {
        let upserted_id = dynamic_postgres_upsert!(backtest_case, "t_backtest_case", self.inner.get_pool())?;
        info!("Inserted backtest_case.id: {:?}", upserted_id);
        Ok(upserted_id)
    }

    async fn update(&self, mut backtest_case: BacktestCaseInfo) -> Result<i64, Error> {
        let updated_id = dynamic_postgres_update!(backtest_case, "t_backtest_case", self.inner.get_pool())?;
