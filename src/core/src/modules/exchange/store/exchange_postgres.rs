...
pub struct ExchangeInfoPostgresRepository {
    inner: PostgresRepository<ExchangeInfo>,
}
...
impl IAsyncRepository<ExchangeInfo> for ExchangeInfoPostgresRepository {
    async fn select(
        ...
        // let serialized = serde_json::to_value(&exchange)?;
        // let obj = serialized
        //     .as_object()
        ...
        //     }
        // }
        // if let Some(id) = exchange.base.id {
        ...
        //     page.get_offset()
        // );
        // let mut operator = sqlx::query_as::<_, ExchangeInfo>(&query);
        // for param in params.iter() {
        //     if let GenericValue::Bool(v) = param {
    ...
    async fn update(&self, mut exchange: ExchangeInfo) -> Result<i64, Error> {
        let updated_id = dynamic_postgres_update!(exchange, "t_exchange", self.inner.get_pool())?;
