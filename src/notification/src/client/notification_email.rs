...
impl std::fmt::Display for SigbotEmailClientConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "(id={}, smtp_host={:?}, smtp_port={:?}, smtp_username={:?}, smtp_password_len={:?})",
            self.id,
            self.smtp_host,
            self.smtp_port,
            self.smtp_username,
...
impl SigbotEmailClientConfig {
    pub fn from_config(notification: Arc<NotificationInfo>) -> Arc<Self> {
        ...
        Arc::new(Self {
            ...
            smtp_host: plain_config
                .get("smtp_host")
                .expect("smtp_host is required")
                .to_string(),
            ...
            smtp_username: plain_config
                .get("smtp_username")
                .expect("smtp_username is required")
                .to_string(),
            smtp_password: secret_config
                .get("smtp_password")
                .expect("smtp_password is required")
                .to_string(),
            from_address: plain_config
                .get("from_address")
                .expect("from_address is required")
                .to_string(),
...
impl ISigbotNotificationClient for SigbotEmailClient {
    ...
    async fn init(&self) {
        ...
        match AsyncSmtpTransport::<Tokio1Executor>::relay(&self.config.smtp_host) {
            Ok(builder) => {
                let mailer = builder.port(self.config.smtp_port).credentials(creds).build();

                *self.smtp_transport.lock().await = Some(mailer);
                info!(
                    "Initialized Email notification with smtp_host={}, smtp_port={}",
                    self.config.smtp_host, self.config.smtp_port
                );
            }
            Err(e) => {
                warn!("Failed to create SMTP relay for host {}: {}", self.config.smtp_host, e);
            }
        }
    }
