...
function usages() {
    echo $"SigBot Development and Building and Deployment fast Tooling.
# for examples
...
function deploy::docker_compose() {
    ...
    if [ ! -f "$compose_file" ]; then
        local remote_compose_file="https://raw.githubusercontent.com/wl4g/sigbot/main/tooling/deploy/compose/docker-compose.yml"
...
function deploy::docker_compose::prune_all_volumes() {
    ...
    if [ -z "$($compose_cmd -f $compose_file ls | grep sigbot)" ]; then
        set +e
        #docker volume rm sigbot_zookeeper_data 2>/dev/null
        #docker volume rm sigbot_kafka_data 2>/dev/null
        #docker volume rm sigbot_minio_data 2>/dev/null
        #docker volume rm sigbot_mongodb_data 2>/dev/null
        docker volume rm sigbot_redis_data_0 2>/dev/null
        docker volume rm sigbot_redis_data_1 2>/dev/null
        docker volume rm sigbot_redis_data_2 2>/dev/null
        docker volume rm sigbot_redis_data_3 2>/dev/null
        docker volume rm sigbot_redis_data_4 2>/dev/null
        docker volume rm sigbot_redis_data_5 2>/dev/null
        set -e
    else
...
case $1 in
  ...
  build-image)
    case $2 in
        -b|--backend)
            build::docker_image "sigbot" "Dockerfile"
            ;;
        -d|--initdb)
            build::docker_image "sigbot-initdb" "Dockerfile.initdb"
            ;;
        -A|--all)
            build::docker_image "sigbot" "Dockerfile"
            build::docker_image "sigbot-initdb" "Dockerfile.initdb"
  ...
  push-image)
    case $2 in
        -b|--backend)
            build::push_image "sigbot"
            ;;
        -d|--initdb)
            build::push_image "sigbot-initdb"
            ;;
        -A|--all)
            build::push_image "sigbot"
            build::push_image "sigbot-initdb"
  ...
  build-push)
    build::docker_image "sigbot" "Dockerfile"
    build::docker_image "sigbot-initdb" "Dockerfile"
    #build::docker_image "sigbot-ui" "Dockerfile.ui"
    build:push_image "sigbot"
    build:push_image "sigbot-initdb"
    ;;
  prune-image)
  ...
  deploy-docker)
    case $2 in
        -S|--status)
            docker ps --format "table {{.ID}} {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}" | grep sigbot_
            ;;
        -U|--up)
