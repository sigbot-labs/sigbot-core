...
impl SigbotMessagerClientFactory {
    ...
    pub async fn init(
        ...
        info!("Registering Sigbot Messager: {}", &provider.as_str());
        ...
        info!("Initializing the messager with provider: {}", &provider.as_str());
        registered.init().await;
        info!("Initialized the messager with provider: {}.", &provider.as_str());
