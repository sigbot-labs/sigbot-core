...
pub async fn auth_middleware(State(state): State<SigBotState>, req: Request<Body>, next: Next) -> impl IntoResponse {
    let uri = req.uri();
...
async fn validate_token(state: &SigBotState, ak: &str) -> (bool, Option<AuthUserClaims>) {
...
async fn handle_connect_oidc(State(state): State<SigBotState>, headers: header::HeaderMap) -> impl IntoResponse {
...
async fn handle_connect_github(State(state): State<SigBotState>, headers: header::HeaderMap) -> impl IntoResponse {
        ...
        Some(client) => {
            ...
            match token_result {
                Ok(token_response) => {
                    ...
                    // let claims = match id_token.claims(&client.id_token_backtest(), &nonce) {
                    //     Ok(claims) => claims,
                    //     Err(e) => {
                    //         return auths::auth_resp_redirect_or_json(
                    //             &state.config,
