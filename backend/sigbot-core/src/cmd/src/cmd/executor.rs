...
pub struct SigbotExecutorServer {}

impl SigbotExecutorServer {
    pub const COMMAND_NAME: &'static str = "executor";

    pub fn build() -> Command {
        Command::new(Self::COMMAND_NAME).about("Run SigBot based on AI LLM + Vector DB ModSec rules Executor.")
    }
    ...
    async fn start(config: &Arc<AppConfig>, verbose: bool) {
        ...
        let listener = match TcpListener::bind(&bind_addr).await {
            Ok(l) => {
                tracing::info!("SigBot Executor server is ready on {}", bind_addr);
                l
            }
            Err(e) => {
        ...
        {
            Ok(_) => {
                tracing::info!("SigBot Executor server shut down gracefully");
            }
            Err(e) => {
                tracing::error!("Error running web server: {}", e);
                panic!("Error start SigBot Executor server: {}", e);
            }
        }
    }
 ...
 ____                                  __                   
/\  _`\                               /\ \__                
\ \ \L\_\  __  _    __    ___   __  __\ \ ,_\   ___   _ __  
 \ \  _\L /\ \/'\ /'__`\ /'___\/\ \/\ \\ \ \/  / __`\/\`'__\
  \ \ \L\ \/>  <//\  __//\ \__/\ \ \_\ \\ \ \_/\ \L\ \ \ \/ 
