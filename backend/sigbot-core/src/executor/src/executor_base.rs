// SPDX-License-Identifier: GNU GENERAL PUBLIC LICENSE Version 3
//
// Copyleft (c) 2024 James Wong. This file is part of James Wong.
// is free software: you can redistribute it and/or modify it under
// the terms of the GNU General Public License as published by the
// Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// James Wong is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with James Wong.  If not, see <https://www.gnu.org/licenses/>.
//
// IMPORTANT: Any software that fully or partially contains or uses materials
// covered by this license must also be released under the GNU GPL license.
// This includes modifications and derived works.

use crate::executor_simple_llm::SimpleLLMExecutor;
use anyhow::Error;
use async_trait::async_trait;
use common_telemetry::info;
use lazy_static::lazy_static;
use serde::{Deserialize, Serialize};
use sigbot_core::config::config;
use std::{
    collections::HashMap,
    sync::{Arc, RwLock},
};

#[async_trait]
pub trait ISigBotExecutor: Send + Sync {
    async fn init(&self);
}

lazy_static! {
    static ref SINGLE_INSTANCE: RwLock<SigBotExecutorManager> = RwLock::new(SigBotExecutorManager::new());
}

pub struct SigBotExecutorManager {
    pub implementations: HashMap<String, Arc<dyn ISigBotExecutor + Send + Sync>>,
}

impl SigBotExecutorManager {
    fn new() -> Self {
        SigBotExecutorManager {
            implementations: HashMap::new(),
        }
    }

    pub fn get() -> &'static RwLock<SigBotExecutorManager> {
        &SINGLE_INSTANCE
    }

    pub async fn init() {
        info!("Register All SigBot executors ...");

        for config in &config::get_config().services.executors {
            if !config.enabled {
                info!("Skipping implementation executor: {}", config.name);
                continue;
            }
            // TODO: Full use similar java spi provider mechanism.
            if config.kind == SimpleLLMExecutor::KIND {
                match Self::get()
                    .write() // If acquire fails, then it block until acquired.
                    .unwrap() // If acquire fails, then it should panic.
                    .register(config.kind.to_owned(), SimpleLLMExecutor::new(config).await)
                {
                    Ok(registered) => {
                        info!("Initializing SigBot Executor ...");
                        let _ = registered.init().await;
                    }
                    Err(e) => panic!("Failed to register SigBot Executor: {}", e),
                }
            }
        }
    }

    fn register<T: ISigBotExecutor + Send + Sync + 'static>(
        &mut self,
        name: String,
        handler: Arc<T>,
    ) -> Result<Arc<T>, Error> {
        if self.implementations.contains_key(&name) {
            tracing::debug!("Already register the Executor '{}'", name);
            return Ok(handler);
        }
        self.implementations.insert(name, handler.to_owned());
        Ok(handler)
    }

    pub async fn get_implementation(name: String) -> Result<Arc<dyn ISigBotExecutor + Send + Sync>, Error> {
        // If the read lock is poisoned, the program will panic.
        let this = SigBotExecutorManager::get().read().unwrap();
        if let Some(implementation) = this.implementations.get(&name) {
            Ok(implementation.to_owned())
        } else {
            let errmsg = format!("Could not obtain registered Executor '{}'.", name);
            return Err(Error::msg(errmsg));
        }
    }
}

#[derive(Clone, Serialize, Deserialize)]
pub struct SigbotAccessEvent {
    // Request information.
    pub method: String,
    pub scheme: Option<String>,
    pub host: Option<String>,
    pub port: Option<u16>,
    pub headers: Option<HashMap<String, Option<String>>>,
    pub path: String,
    pub query: Option<String>,
    pub body: Option<String>,
    // Additional request information.
    pub req_id: Option<String>,
    pub client_ip: Option<String>,
    pub start_time: u64,
    // Response information.
    pub resp_status_code: Option<i32>,
    pub resp_headers: Option<HashMap<String, Option<String>>>,
    pub resp_body: Option<String>,
    // Additional response information.
    pub duration: Option<u64>,
}
