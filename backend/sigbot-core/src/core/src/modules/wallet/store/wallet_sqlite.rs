...
pub struct WalletInfoSQLiteRepository {
    inner: SQLiteRepository<WalletInfo>,
}

impl WalletInfoSQLiteRepository {
    pub async fn new(config: &SqliteAppDBProperties) -> Result<Self, Error> {
        Ok(WalletInfoSQLiteRepository {
...
impl AsyncRepository<WalletInfo> for WalletInfoSQLiteRepository {
    ...
    async fn select_by_id(&self, id: i64) -> Result<WalletInfo, Error> {
        let wallet = sqlx::query_as::<_, WalletInfo>("SELECT * FROM s_wallets WHERE id = ? AND del_flag = 0")
            .bind(id)
    ...
    async fn update(&self, mut wallet: WalletInfo) -> Result<i64, Error> {
        let updated_id = dynamic_sqlite_update!(wallet, "s_wallets", self.inner.get_pool())?;
        info!("Updated wallet.id: {:?}", updated_id);
        Ok(updated_id)
    }

    async fn delete_all(&self) -> Result<u64, Error> {
        let delete_result = sqlx::query("UPDATE s_wallets SET del_flag = 1")
    ...
    async fn delete_by_id(&self, id: i64) -> Result<u64, Error> {
        let delete_result = sqlx::query("UPDATE s_wallets SET del_flag = 1 WHERE id = ? AND del_flag = 0")
            .bind(id)
            .execute(self.inner.get_pool())
            .await?;
        info!("Deleted result: {:?}", delete_result);
