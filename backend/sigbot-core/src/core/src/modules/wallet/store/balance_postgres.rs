...
pub struct BalanceInfoPostgresRepository {
    inner: PostgresRepository<BalanceInfo>,
}

impl BalanceInfoPostgresRepository {
    pub async fn new(config: &PostgresAppDBProperties) -> Result<Self, Error> {
        Ok(BalanceInfoPostgresRepository {
...
impl AsyncRepository<BalanceInfo> for BalanceInfoPostgresRepository {
    async fn select(&self, balance: BalanceInfo, page: PageRequest) -> Result<(PageResponse, Vec<BalanceInfo>), Error> {
        ...
        if !balance.asset.is_empty() {
            conditions.push(format!("asset = ${}", param_index));
            param_index += 1;
        }
        ...
        if balance.wallet_id != 0 {
            count_query_builder = count_query_builder.bind(balance.wallet_id);
        }
        if !balance.asset.is_empty() {
            count_query_builder = count_query_builder.bind(&balance.asset);
        }
        ...
        if balance.wallet_id != 0 {
            query_builder = query_builder.bind(balance.wallet_id);
        }
        if !balance.asset.is_empty() {
            query_builder = query_builder.bind(&balance.asset);
        }
    ...
    async fn select_by_id(&self, _id: i64) -> Result<BalanceInfo, Error> {
        // s_balances uses composite primary key (wallet_id, asset), use select_by_wallet_and_asset instead
        Err(Error::msg(
    ...
    async fn update(&self, balance: BalanceInfo) -> Result<i64, Error> {
        sqlx::query(
            "UPDATE s_balances SET available = $1, locked = $2, updated_at = NOW() WHERE wallet_id = $3 AND asset = $4",
        )
        .bind(balance.available)
        .bind(balance.locked)
        .bind(balance.wallet_id)
    ...
    async fn delete_by_id(&self, id: i64) -> Result<u64, Error> {
        ...
        let delete_result = sqlx::query("DELETE FROM s_balances WHERE wallet_id = $1")
            .bind(id)
            .execute(self.inner.get_pool())
            .await?;
        info!("Deleted result: {:?}", delete_result);
