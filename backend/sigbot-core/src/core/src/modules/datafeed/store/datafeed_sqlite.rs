...
pub struct DatafeedInfoSQLiteRepository {
    inner: SQLiteRepository<DatafeedInfo>,
}

impl DatafeedInfoSQLiteRepository {
    pub async fn new(config: &SqliteAppDBProperties) -> Result<Self, Error> {
        Ok(DatafeedInfoSQLiteRepository {
...
impl AsyncRepository<DatafeedInfo> for DatafeedInfoSQLiteRepository {
    ...
    async fn update(&self, mut datafeed: DatafeedInfo) -> Result<i64, Error> {
        let updated_id = dynamic_sqlite_update!(datafeed, "t_datafeed", self.inner.get_pool())?;
