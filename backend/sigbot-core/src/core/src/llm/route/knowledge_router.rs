...
async fn handle_knowledge_upload(State(state): State<SigbotState>, mut multipart: Multipart) -> impl IntoResponse {
    // Create temp directory for uploaded files
    let temp_dir = std::env::var("TEMP_FILE_DIR").unwrap_or_else(|_| "/tmp/sigbot/knowledge_upload".to_string());
