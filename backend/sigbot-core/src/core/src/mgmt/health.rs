...
pub(crate) trait HealthChecker: Send + Sync {
    async fn check(&self, state: &SigbotState) -> HealthCheckResult;
}
...
impl SQLiteChecker {
    ...
    async fn is_sqlite_connected(&self, state: &SigbotState) -> bool {
...
impl MongoChecker {
    ...
    async fn is_mongo_connected(&self, state: &SigbotState) -> bool {
        match &state.config.appdb.db_type {
            AppDBType::MONGODB => {
                let repo = state.user_repo.lock().await;
                match repo
...
async fn handle_healthz(State(state): State<SigbotState>) -> impl IntoResponse {
